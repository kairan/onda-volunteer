...
- **2026-07-28 (#175 Phase 5 cutover Execute):** Production frontend → `@onda/web-onda` only. Deleted `apps/web` + `apps/web-next`; CI/root scripts retargeted; redirects runbook; `frontend-migration-web-next` TLC archived; design.md §9 signed. Record: `docs/issues/done/175-web-onda-phase-5-cutover.md`. **Next:** Verifier + PR merge; then archive `frontend-restart-serve-well-base` per AD-001.
