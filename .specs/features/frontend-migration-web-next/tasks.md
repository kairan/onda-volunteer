...
**What**: Replace the `/dashboard` (Volunteer) and `/scheduling` (Leader) route **stubs** with mock-data previews that render the real Onda components (T03 primitives + T12 `AppShell`) against **hardcoded fixtures** — greeting + a few assignment cards, a roster section with a fill badge, an empty state, and a skeleton — with **no** `apiClient` / TanStack Query wiring. Purpose: a brand look-and-feel checkpoint that closes Slice 1 (#143) before the ~17 live data-backed tasks. Fixtures live in a clearly-labelled `__preview__` location and are **deleted/replaced** by T16 (volunteer) and T20/T21 (leader). Scope is the **two designed roles only** — do not preview admin / system-admin (MIG-ADMIN-01 neutral port, no Onda design).
