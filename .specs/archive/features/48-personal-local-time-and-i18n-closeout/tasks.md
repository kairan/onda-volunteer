...
- [x] T48-01: Inventory remaining shell/dashboard/scheduling literals and missing i18n keys.
  - Verify: key coverage checklist created for targeted surfaces.
- [x] T48-02: Implement remaining translation key wiring for `pt-BR` and `en`.
  - Verify: unit/integration checks confirm locale string resolution.
- [x] T48-03: Implement/refine personal-local time toggle rendering in event/assignment views.
  - Verify: integration tests cover canonical + personal-local dual display.
- [x] T48-04: Implement/refine local/session persistence for time-display preference.
  - Verify: tests confirm preference survives reload within chosen persistence boundary.
- [x] T48-05: Validate fallback behavior (`pt-BR -> en`) and error-safe rendering for missing keys.
  - Verify: i18n fallback tests pass.
- [x] T48-06: Run regression checks that UTC persistence and API payload semantics are unchanged.
