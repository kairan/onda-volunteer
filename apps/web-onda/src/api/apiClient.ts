...
export async function buildProtectedHeaders(
  ...
  const token = options?.bearerAccessToken ?? (await getAccessToken());
  if (token) {
...
export async function getJson<T>(path: string, scope: ProtectedScope): Promise<T> {
  return fetchJsonWithProtectedHeaders<T>(resolveUrl(path), scope);
}
