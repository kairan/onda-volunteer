...
- **2026-06-25 (#148 T30a + T27 on branch `chore/rename-web-to-web-legacy`):** T30a — `apps/web` → `apps/web-legacy`, CI job renames. T27 — `pnpm test:coverage` includes `@onda/web-next`; Vitest floors in `apps/web-next/vitest.config.ts` (64/64/53/61 actual; functions floor 60). T29 blocked (no deploy config). T30 deferred (user: keep `web-next` name). PR pending.

- **2026-06-25 (#148 T30a — legacy app rename):** Renamed `apps/web` → `apps/web-legacy`, package `@onda/web` → `@onda/web-legacy`, root scripts (`dev:web-legacy`, `typecheck:web-legacy`, `test:e2e:web-legacy`), CI jobs (`typecheck-web-legacy`, `playwright-web-legacy`). `apps/web-next` unchanged. Frees `apps/web` path for T30 final cutover. Branch: `chore/rename-web-to-web-legacy` (pending PR). TLC: `tasks.md` T30a checked.
