# 49 — API: request-scoped auth context (tech debt)
...
- **After** slice **47** (HOPE release gate) **or** when HITL slices **40–45** repeatedly duplicate header forwarding.

## Blocked by

None — but avoid running in parallel with AFK slices **35–39** to reduce merge conflict risk.
