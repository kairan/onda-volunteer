...
- **2026-06-06 (Planning round — 4 new feature specs created):** TLC Specify + Design completed for three product features and one doc-closeout housekeeping feature. Execute order: (1) `org-structure-doc-closeout` pre-Execute tasks (doc sync, runs now); (2) #108 Execute (`organization-structure-administration` tasks.md Phase 4); (3) `leader-roster-assignment-ui`; (4) `event-edit-reschedule`; (5) `volunteer-onboarding-invite`. See `.specs/features/` for all artifacts and ROADMAP.md for priority order.
